IRC is used by Verocoin server to find 'peers' - other Verocoin servers. The
current list can be seen by running:

  verocoin-server peers

The following config file options are used by the IRC part of Verocoin server:
...
the Verocoin server will run in private mode (default). In private
mode, `verocoin-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Verocoin
...
your Verocoin server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #verocoin.
